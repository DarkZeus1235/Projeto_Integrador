...
if (isset($_GET['codigo_bebida'])) {
    $id_uisque = $_GET['codigo_bebida'];
    $sql_consultar = "SELECT * FROM cadastro_uisque WHERE id_uisque = '$id_uisque'";
    $comando_sql = $mysqli->query($sql_consultar) or die($mysqli->error);
    $bebidas = $comando_sql->fetch_assoc();

    if (isset($_POST['btn_deletar'])) {

        $sql_deletar = "DELETE FROM cadastro_uisque WHERE id_uisque = '$id_uisque'";

        $deu_certo = $mysqli->query($sql_deletar) or die($mysqli->error);

        header("location:lista_uisque.php");
...
<body>
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <a class="btn btn-warning" href="lista_uisque.php">Voltar</a>
        </form>

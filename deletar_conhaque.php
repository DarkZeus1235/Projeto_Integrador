...
if (isset($_GET['codigo_bebida'])) {
    $id_conhaque = $_GET['codigo_bebida'];
    $sql_consultar = "SELECT * FROM cadastro_conhaque WHERE id_conhaque = '$id_conhaque'";
    $comando_sql = $mysqli->query($sql_consultar) or die($mysqli->error);
    $bebidas = $comando_sql->fetch_assoc();

    if (isset($_POST['btn_deletar'])) {

        $sql_deletar = "DELETE FROM cadastro_conhaque WHERE id_conhaque = '$id_conhaque'";

        $deu_certo = $mysqli->query($sql_deletar) or die($mysqli->error);

        header("location:lista_conhaque.php");
...
<body>
    <div class="container">
        ...
        <p>Arquivo_Caminho:
            <?php echo $bebidas['arquivo_caminho'] ?>
        </p>
        <p>Valor:
        ...
        <form action="" method="post">
            ...
            <a class="btn btn-warning" href="lista_conhaque.php">Voltar</a>
        </form>

...
if (isset($_GET['codigo_bebida'])) {
    $id_vodka = $_GET['codigo_bebida'];
    $sql_consultar = "SELECT * FROM cadastro_vodka WHERE id_vodka = '$id_vodka'";
    $comando_sql = $mysqli->query($sql_consultar) or die($mysqli->error);
    $bebidas = $comando_sql->fetch_assoc();

    if (isset($_POST['btn_deletar'])) {

        $sql_deletar = "DELETE FROM cadastro_vodka WHERE id_vodka = '$id_vodka'";

        $deu_certo = $mysqli->query($sql_deletar) or die($mysqli->error);

        header("location:lista_vodka.php");
...
<body>
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <a class="btn btn-warning" href="lista_vodka.php">Voltar</a>
        </form>

<?php
    ...
    if(isset($_GET['codigo_bebida'])){
        $id_champ = $_GET['codigo_bebida'];
        $sql_consultar = "SELECT * FROM cadastro_champ WHERE id_champ = '$id_champ'";
        $comando_sql = $mysqli->query($sql_consultar) or die($mysqli->error);
        $bebidas = $comando_sql->fetch_assoc();

        if(isset($_POST['btn_deletar'])){ 
    
            $sql_deletar = "DELETE FROM cadastro_champ WHERE id_champ = '$id_champ'";
    
            $deu_certo = $mysqli->query($sql_deletar) or die ($mysqli->error);

            header("location:lista_champ.php");
...
<html lang="pt-br">
    ...
    <body>
        <div class="container">
            ...
            <h1>ID da Bebida: <?php echo $bebidas['id_champ']?></h1>
            ...
            <form action="" method="post">
                ...
                <a class="btn btn-warning" href="lista_champ.php">Voltar</a>
            </form>
